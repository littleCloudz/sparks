...
###### Accept
告诉服务器能够发送哪些媒体类型  
下面的 Accept 首部就用来告知服务器客户端会接受与其请求相符的任意媒体类型：  
Accept: \*/\*  
...
###### Set-Cookie2
与Set-Cookie类似，RFC 2965 Cookie定义；参见11.6.7节
...
这个主体的对象类型
可以用实体首部来说明实体主体部分的数据类型。    
说明了主体是什么——在这个例子中，就是纯文本文档。  
<span class="s1">Content-Type</span>: text/html; charset=iso-latin-1  
通过 <span class="s1">Content-Type</span> 首部告知应用程序，数据是以 iso-latin-1 字符集表示的 HTML 文档  
<span class="s1">Content-Type</span>: image/gif  
实体的主体部分是一个GIF图片  
...
![](20200429/.http_images/8cb28182.png)
...
![](20200429/.http_images/d7cc16eb.png)	
